...
class AIService:
    def __init__(self):
        ...
        self.openai_client = AsyncOpenAI(base_url=str(settings.OPENAI_BASE_URL), api_key=settings.OPENAI_API_KEY)
    ...
    def detect_tool_calls(self, content: str) -> Optional[List[MCPToolCall]]:
        ...
        if "calculate" in content.lower() and any(op in content for op in ["+", "-", "*", "/"]):
            return [MCPToolCall(
